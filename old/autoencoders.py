...
#####################################################################
    ...
    def propagation(self, fInput, fDropout=False):
        ...
        the neural network. A dropout system can be activated. 
...
#####################################################################
    ...
    def layer_error(self, fOut, fIn, fSparsity=False):
        ...
        try:
            ...
            np.savetxt("log/error_dsig.log", self.mDerived[-1](fOut[-1]))
...
#####################################################################
    ...
    def angle_driven_approach(self, fWgrad):
        ...
        for i in xrange(self.mNbLayers - 1):
            ...
            self.mEpsilon[i]  = min(self.mEpsilon[i], LIMIT)
            self.mEpsilon[i]  = max(self.mEpsilon[i],-LIMIT)
            ...
            try:
                ...
                self.mMomentum[i]  = min(self.mMomentum[i], LIMIT)
                self.mMomentum[i]  = max(self.mMomentum[i],-LIMIT)
...
#####################################################################
    ...
    def train(self, fImgs, fLbls, fIterations, fName):
        ...
        for i in xrange(fIterations):
            ...
            # Stop condition
            if i > 0 and abs(_gcost[i-1] - _gcost[i]) < CONVERGENCE:
                _done = i + 1
                break
